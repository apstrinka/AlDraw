...
{
	private AlDrawState prev;
	private AlDrawState next;
	...
	private double mark;
	private AlDrawController controller;
	...
	public void start()
	{
		...
		double initRad = 1;
		circles.add(new Circle(new DPoint(0.0, 0.0), initRad));
		for (int i = 0; i < 6; i++)
		{
			points.add(new DPoint(initRad * Math.cos(i * Math.PI / 3), initRad * Math.sin(i * Math.PI / 3)));
		}
	}
	
	public void setController(AlDrawController c){
		controller = c;
	}
	
	public AlDrawState getPrev()
	{
		return prev;
	}

	public void setPrev(AlDrawState prev)
	{
		this.prev = prev;
	}
	...
	public void setNext(AlDrawState next)
	{
		this.next = next;
	}
	...
	public void paintState(Canvas canvas, Paint paint, CoordinateConverter c, Context context, boolean showLines, boolean showPoints)
	{
		for (int i = 0; i < this.getNumEnclosures(); i++)
		{
			Enclosure e = this.getEnclosure(i);
			e.draw(canvas, paint, c, context);
		}
		...
		if (showLines){
			...
			for (int i = 0; i < this.getNumSegments(); i++)
			{
				...
				l.draw(canvas, paint, c, context);
			}
			for (int i = 0; i < this.getNumRays(); i++)
			{
				Ray r = this.getRay(i);
				r.draw(canvas, paint, c, context);
			}
			for (int i = 0; i < this.getNumLines(); i++)
			{
				Line l = this.getLine(i);
