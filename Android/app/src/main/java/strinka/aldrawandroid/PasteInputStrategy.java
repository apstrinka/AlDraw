...
{
	public PasteInputStrategy(AlDrawController controller)
	{
		super("Paste", 2, controller);
	}

	@Override
	void endHook()
	{
		controller.copy(controller.getSelPoint(0), controller.getSelPoint(1));
	...
	ArrayList<Drawable> shadowHook(DPoint p1)
	{
		if (controller.getNumCopy() > 1)
		{
			...
			for (int i = 0; i < selected.size(); i++)
			{
				...
				shadows.add(s.copy(controller.getCopyPoint(0), p1));
			}
			return shadows;
		}
	...
	ArrayList<Drawable> shadowHook(DPoint p1, DPoint p2)
	{
		if (controller.getNumCopy() > 1)
		{
			...
			{
				Selectable s = selected.get(i);
				shadows.add(s.copy(controller.getCopyPoint(0), controller.getCopyPoint(1), p1, p2));
			}
			return shadows;
		}
		return new ArrayList<Drawable>();
