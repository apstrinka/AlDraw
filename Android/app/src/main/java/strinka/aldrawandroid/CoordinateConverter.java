...
public class CoordinateConverter
{
	...
	{
		setConversionRatio(Math.min(width, height)/2);
	}
	...
	public void zoom(DPoint firstAbstract, DPoint secondAbstract, DPoint firstScreen, DPoint secondScreen, boolean lockAngle){
		...
		if (!lockAngle){
			...
			angle = Utils.angleDifference(screenAngle, abstractAngle);
		}
		conversionRatio = screenDist/abstractDist;
		DPoint diff = screenToAbstractCoord(firstScreen);
		double dx = diff.getX() - firstAbstract.getX();
		double dy = diff.getY() - firstAbstract.getY();
		cX = cX - dx;
		cY = cY - dy;
	}
